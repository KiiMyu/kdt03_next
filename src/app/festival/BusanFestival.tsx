...
function BusanFestival() {
    ...
    const location = /*useLocation()*/useSearchParams();
    const gugunItem = /*location.state*/location.get('gugun') || '';
    console.log(gugunItem)

    const RenewCard = async () => {

        let url = `https://apis.data.go.kr/6260000/FestivalService/getFestivalKr?serviceKey=${process.env.NEXT_PUBLIC_ACCIDENT_API}&pageNo=1&numOfRows=45&resultType=json&type=json`
    ...
    useEffect(() => {
        ...
        setGulist(tm);
        if (gugunItem != '') {
            console.log("gugunItem:", gugunItem);
            ...
            <div className='p-5 grid grid-cols-3 overflow-y-scroll'>
                {
                    jsonData && jsonData.getFestivalKr.item.map((item, index: number) => (
                        selectCountry == '' || selectCountry == item.GUGUN_NM ?
                            <Link href={`/festival/content?item=${encodeURIComponent(JSON.stringify(item))}`} key={item.UC_SEQ + index}>
